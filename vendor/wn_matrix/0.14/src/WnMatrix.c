...
WnMatrix *WnMatrix__new( size_t i_rows, size_t i_columns) {
  ...
  for (i = 0; i< self->iMax; ++i) {
    xmlStrPrintf( sx_tmp, WNMATRIX_BUF_SIZE, (const char *) "%lu", i+1 );
    memcpy(
...
)
{
  ...
  xmlStrPrintf( sx_row, WNMATRIX_BUF_SIZE, (const char *) "%lu", i_row );
  xmlStrPrintf( sx_col, WNMATRIX_BUF_SIZE, (const char *) "%lu", i_col );
...
xmlDocPtr WnMatrix__make_gsl_vector_xml_document(
  gsl_vector *self, const char *sx_format
)
{
